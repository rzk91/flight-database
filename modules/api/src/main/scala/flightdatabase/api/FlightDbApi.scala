...
import flightdatabase.api.syntax.kleisli._
...
class FlightDbApi[F[_]: Async] private (apiConfig: ApiConfig, repos: Repositories[F]) {
...
object FlightDbApi {
  ...
  def apply[F[_]: Async](apiConfig: ApiConfig, repos: Repositories[F]): FlightDbApi[F] =
    new FlightDbApi(apiConfig, repos)
}
