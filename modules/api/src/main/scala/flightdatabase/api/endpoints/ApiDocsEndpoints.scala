...
// endpoint's prefix, sourced entirely from the classpath (the scalar-core webjar, no CDN) so
// it works fully offline once built.
class ApiDocsEndpoints[F[_]: Sync] private (prefix: String) extends Endpoints[F](prefix) {

  private val webjarBase = "META-INF/resources/webjars/scalar"
  ...
  override val endpoints: HttpRoutes[F] = HttpRoutes.of {
    ...
    // GET /docs/{asset...} -> the Scalar webjar's static assets, e.g. scalar.js. Must stay
    // last: it matches any remaining path.
    case req @ GET -> path =>
...
object ApiDocsEndpoints {
  ...
  def apply[F[_]: Sync](prefix: String): Endpoints[F] =
    new ApiDocsEndpoints[F](prefix)
}
