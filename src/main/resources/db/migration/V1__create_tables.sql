...
CREATE TABLE fleet (
	...
	iso2 character varying (2) NOT NULL CHECK (btrim(iso2::text) <> ''::text),
	iso3 character varying (3) NOT NULL CHECK (btrim(iso3::text) <> ''::text),
	call_sign character varying NOT NULL,
	hub_airport_id bigint NOT NULL REFERENCES airport (id) ON DELETE CASCADE,
	created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
	last_updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);
...
CREATE TABLE fleet_route (
	id bigserial NOT NULL,
	PRIMARY KEY (id),
	fleet_airplane_id bigint NOT NULL REFERENCES fleet_airplane (id) ON DELETE CASCADE,
